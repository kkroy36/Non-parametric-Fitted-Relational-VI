...
class FVI(object):
    ...
    def get_immediate_reward(self,state_id="no_id",action=action):
        '''immediate reward model'''
        
        if state_id != "last_but_one":
        ...
        if not AVI:  # if not AVI i.e. for first iteration perform value iteration for initial values
            ...
            for i in range(number_of_transitions):
                if i == 0:
                    ...
                    immediate_reward = self.get_immediate_reward(state_id="last_but_one",action=current_action)
                    ...
                    current_state = reversed_trajectory[i][1][:-1]
                    current_action = reversed_trajectory[i][1][-1]
                    immediate_reward = self.get_immediate_reward(action=current_action)
        ...
        for trajectory in trajectories:
            ...
            for i in range(number_of_transitions-1):
                ...
                try:
                    ...
                    #self.print_tree(self.model)
                    #####raw_input("compute_bellman_error")
                immediate_reward = self.get_immediate_reward(action=state_action)    
