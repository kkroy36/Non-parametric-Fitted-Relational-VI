...
class City(object):
    ...
    def remove_truck(self, truck_to_remove):
        ...
        self.trucks = [truck for truck in self.trucks if str(
            truck) != str(truck_to_remove)]
    ...
    def remove_unloaded_box(self, b):
        self.unloaded_boxes = [
            box for box in self.unloaded_boxes if str(box) != str(b)]
...
class Truck(object):
    ...
    def remove_box(self, box_to_remove):
        self.boxes = [box for box in self.boxes if str(
            box) != str(box_to_remove)]
...
class Logistics(object):  # represents a world state
    ...
    bk = ["bOn(+state,+box,+truck)",
          ...
          "load(state,truck,box)",
          "unload(state,truck,box)",
          "move(state,truck,city)"]
    ...
    def get_move_combinations(self, trucks, cities):
        ...
        for truck in trucks:
            for city in cities:
                combinations.append(
                    "move(s"+str(self.state_number)+","+str(truck)+","+str(city)+").")
        return combinations
    ...
    def get_load_combinations(self, trucks):
        ...
        for truck in trucks:
            for box in boxes:
                combinations.append(
                    "load(s"+str(self.state_number)+","+str(box)+","+str(truck)+").")
        return combinations
    ...
    def get_unload_combinations(self, trucks):
        ...
        for truck in trucks:
            ...
            for box in boxes:
                combinations.append(
                    "unload(s"+str(self.state_number)+","+str(box)+","+str(truck)+").")
        return combinations
    ...
    def get_state_facts(self):
        ...
        for city in self.cities:
            for truck in city.get_trucks():
                for box in truck.get_boxes():
                    box_fact = "bOn(s"+str(self.state_number) + \
                        ","+str(box)+","+str(truck)+")"
                    facts.append(box_fact)
                truck_fact = "tIn(s"+str(self.state_number) + \
                    ","+str(truck)+","+str(city)+")"
                facts.append(truck_fact)
            if str(city) == "c3":  # destination city
                city_fact = "destination(s" + \
                    str(self.state_number)+","+str(city)+")"
                facts.append(city_fact)
            for box in city.unloaded_boxes:
                city_fact = "bIn(s"+str(self.state_number) + \
                    ","+str(box)+","+str(city)+")"
                facts.append(city_fact)
        return facts
